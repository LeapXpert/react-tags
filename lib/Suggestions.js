...
class Suggestions extends Component {
  ...
  componentDidUpdate = prevProps => {
    ...
    if (suggestionsContainer && prevProps.selectedIndex !== selectedIndex) {
      const activeSuggestion = suggestionsContainer.querySelector(classNames.activeSuggestion)
    ...
    const suggestions = props.suggestions.map(
      function(item, i) {
        return (
          ...
          >
            <span dangerouslySetInnerHTML={this.markIt(item.text, props.query)} />
            <span className="ReactTags_value">
              {item.value}
            </span>
          </li>
        )
